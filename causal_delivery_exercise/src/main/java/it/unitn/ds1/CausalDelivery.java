...
public class CausalDelivery {
    ...
    private static final int N_LISTENERS = 10; // number of listening actors
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < N_LISTENERS; i++) {
            group.add(
                system.actorOf(Chatter.props(id++, null), "listener" + i)
            );
        }
        ...
        // tell the first chatter to start conversation a
        group.get(0).tell(new StartChatMsg(), null);

        // tell the third chatter to start conversation b
